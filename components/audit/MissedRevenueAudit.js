...
function AnimatedNumber({ value, prefix = '', suffix = '' }) {
  ...
  return <span>{prefix}{Math.round(displayed).toLocaleString('en-AU')}{suffix}</span>
}
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      await fetch('/api/audit', {
        ...
        body: JSON.stringify({ name, email, type: 'missed-revenue', ...results }),
      }).catch(() => {})
  ...
  return (
    <>
      <style>{`
        ...
        .btn-sage:hover { background: #8FAF9F; }
      `}</style>
      ...
      <div style={{ backgroundColor: '#F7F3ED', overflowX: 'hidden' }}>
        ...
        <section style={{
          ...
          <div style={{ position: 'relative', maxWidth: '660px' }}>
            <p style={{ ...monoLabel, color: 'rgba(143,175,159,0.72)', marginBottom: '20px' }}>
        ...
        <section style={{ padding: 'clamp(48px, 6vw, 80px) 24px clamp(80px, 10vw, 120px)' }}>
          <div style={{ maxWidth: '660px', margin: '0 auto' }}>
            ...
            {phase === 'input' && (
              <div style={{
                ...
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ ...monoLabel, color: 'rgba(28,28,26,0.42)', display: 'block', marginBottom: '8px' }}>Business Website URL</label>
                  <input type="url" className="audit-field" placeholder="https://yourbusiness.com.au" value={url} onChange={e => setUrl(e.target.value)} />
                </div>

                {/* Job value + monthly leads */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(190px, 100%), 1fr))', gap: '14px', marginBottom: '28px' }}>
                  <div>
                    <label style={{ ...monoLabel, color: 'rgba(28,28,26,0.42)', display: 'block', marginBottom: '8px' }}>Average Job Value ($)</label>
                    <input type="number" className="audit-field" placeholder="e.g. 850" value={jobValue} onChange={e => setJobValue(e.target.value)} min={0} />
                  </div>
                  <div>
                    <label style={{ ...monoLabel, color: 'rgba(28,28,26,0.42)', display: 'block', marginBottom: '8px' }}>Monthly Lead Volume</label>
                    <input type="number" className="audit-field" placeholder="e.g. 30" value={monthlyLeads} onChange={e => setMonthlyLeads(e.target.value)} min={0} />
                  </div>
                </div>

                {/* Response time */}
                <div style={{ marginBottom: '24px' }}>
                  <label style={{ ...monoLabel, color: 'rgba(28,28,26,0.42)', display: 'block', marginBottom: '10px' }}>How fast do you respond to new leads?</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {RESPONSE_OPTIONS.map(o => (
                      <button key={o.value} className={`pill${responseTime === o.value ? ' on' : ''}`} onClick={() => setResponseTime(o.value)}>{o.label}</button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '36px' }}>
                  <label style={{ ...monoLabel, color: 'rgba(28,28,26,0.42)', display: 'block', marginBottom: '10px' }}>Manual admin time per week</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {ADMIN_OPTIONS.map(o => (
                      <button key={o.value} className={`pill${adminTime === o.value ? ' on' : ''}`} onClick={() => setAdminTime(o.value)}>{o.label}</button>
                    ))}
                  </div>
                </div>
            ...
            {phase === 'scanning' && (
              <div style={{
                backgroundColor: '#1C1C1A', border: '1px solid rgba(143,175,159,0.18)',
            ...
            {phase === 'results' && results && (
              <div ref={resultsRef}>
                {/* Diagnostic card */}
                <div style={{
                  backgroundColor: '#1C1C1A', border: `1px solid ${results.grade.color}30`,
                  borderRadius: '24px', padding: 'clamp(28px, 5vw, 44px)',
                  marginBottom: '14px', boxShadow: '0 8px 48px rgba(28,28,26,0.22)',
                }}>
                  ...
                  {/* 3 stat cards */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(155px, 100%), 1fr))', gap: '10px', marginBottom: '24px' }}>
                    {[
                      { icon: <TrendingDown size={13} color={results.grade.color} />, label: 'Leads lost / mo', display: `${results.leadsLost} of ${results.ml}`, highlight: false },
                      { icon: <DollarSign size={13} color={results.grade.color} />, label: 'Monthly rev. leak', display: `$${results.monthlyRevLeak.toLocaleString('en-AU')}`, highlight: false },
                      { icon: <AlertTriangle size={13} color={results.grade.color} />, label: 'Annual rev. leak', value: results.annualRevLeak, display: null, highlight: true },
                    ].map((s, i) => (
                      <div key={i} style={{
                        ...card(s.highlight ? { background: `${results.grade.color}12`, border: `1px solid ${results.grade.color}28` } : {}),
                      }}>
                ...
                <div className="leak-pulse" style={{
                  backgroundColor: '#1C1C1A', border: `1.5px solid ${results.grade.color}40`,
                  borderRadius: '24px', padding: 'clamp(24px, 4vw, 38px)',
                ...
                {!submitted ? (
                  <div style={{ backgroundColor: '#FDFAF6', border: '1px solid rgba(212,201,176,0.65)', borderRadius: '24px', padding: 'clamp(24px, 4vw, 40px)', boxShadow: '0 4px 32px rgba(28,28,26,0.07)' }}>
                    ...
                    <form onSubmit={handleEmailSubmit}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(175px, 100%), 1fr))', gap: '12px', marginBottom: '12px' }}>
                        <input type="text" className="audit-field" placeholder="First name" value={name} onChange={e => setName(e.target.value)} required />
                        <input type="email" className="audit-field" placeholder="Business email" value={email} onChange={e => setEmail(e.target.value)} required />
                      </div>
                      <button
                        type="submit"
                  ...
                  <div style={{ backgroundColor: '#FDFAF6', border: '1px solid rgba(143,175,159,0.38)', borderRadius: '24px', padding: 'clamp(24px, 4vw, 40px)', textAlign: 'center', boxShadow: '0 4px 32px rgba(143,175,159,0.07)' }}>
