...
import { calcPillarMonthly } from '@/components/audit/calculations.js'
import RadarChart from '@/components/audit/RadarChart.js'
...
function CostReductionGraph() {
  ...
  const filled = circumference * 0.25
  return (
...
function validateAuditData(data) {
  ...
  if (typeof data.b !== 'string' || data.b.length > 200) return null
  if (typeof data.n !== 'string' || data.n.length > 200) return null
  if (data.r !== undefined && typeof data.r !== 'number') return null
  if (data.p !== undefined && typeof data.p !== 'object') return null
  return data
}
