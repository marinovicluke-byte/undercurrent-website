...
import { calcPillarMonthly, calcLeadBleed, calcTotals, calcRating, calcGap, buildPayload } from '@/components/audit/calculations.js'
...
function ReportModal({ isOpen, onClose, payload, pillarsState, industry, customIndustry, hourlyRate, responseTime }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Build the report data payload for URL
    const activePillars = {}
    for (const [key, pillar] of Object.entries(pillarsState)) {
    ...
    const reportData = {
      ...
      i: customIndustry || industry,
      r: hourlyRate,
      rt: responseTime || null,
      p: activePillars,
    }
      ...
      <div style={{
        ...
        <>
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            >
              {status === 'loading' ? 'Loading…' : 'See my full report →'}
            </button>
          </form>
        </>
      </div>
