...
// ─── Small flowing current for process step cards ────────────────────────────
...
// ── Customer Journey: horizontal stage flow ───────────────────────────────────
            ...
            <div key={stage.key} style={{ display: 'flex', alignItems: 'stretch', gap: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 32, flexShrink: 0 }}>
                <div style={{
                  ...
                  flexBasis: i === 0 ? 20 : 0,
                  flexGrow: i === 0 ? 0 : 1,
                  background: isPast ? `rgba(143,175,159,0.5)` : 'rgba(143,175,159,0.12)',
                ...
                <div style={{
                  ...
                  borderRadius: '50%',
                  flexShrink: 0,
                  background: isCurrent ? SAGE : isPast ? 'rgba(143,175,159,0.4)' : 'rgba(143,175,159,0.1)',
                  boxShadow: isCurrent ? `0 0 0 5px rgba(143,175,159,0.18), 0 0 12px ${SAGE}` : 'none',
                  border: isCurrent ? `1.5px solid ${SAGE}` : isPast ? '1px solid rgba(143,175,159,0.4)' : '1px solid rgba(143,175,159,0.15)',
              ...
              <div style={{
                ...
                opacity: isFuture ? 0.28 : isCurrent ? 1 : 0.55,
                ...
                <div style={{
                  ...
                  color: isCurrent ? SAGE : 'rgba(232,224,208,0.85)',
                  lineHeight: 1.3,
                ...
                <div style={{
                  ...
                  fontSize: 10.5,
                  color: isCurrent ? 'rgba(143,175,159,0.7)' : 'rgba(232,224,208,0.3)',
                  marginTop: 3,
...
// ── Sales Pipeline Demo: two-panel terminal + live pipeline ──────────────────
  ...
  useEffect(() => {
    ...
    const showNext = () => {
      ...
      if (idx >= allLines.length) {
        t = setTimeout(() => {
          setLines([])
          setMetrics({ captured: 0, qualified: 0, sequenced: 0, booked: 0 })
          idx = 0
          t = setTimeout(showNext, 800)
        }, 2500)
        return
      }
      ...
      if (line.metric) {
        setMetrics(prev => ({ ...prev, [line.metric]: prev[line.metric] + 1 }))
      }
      idx++
    ...
    <div ref={containerRef} style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 20 }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.28)', borderRadius: 12, padding: '16px 18px', border: '1px solid rgba(143,175,159,0.12)', minHeight: 220 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {lines.map((line, i) => (
            <div key={i} style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, lineHeight: 1.7, color: i === lines.length - 1 ? SAGE : 'rgba(232,224,208,0.38)', transition: 'color 0.3s' }}>{line}</div>
          ))}
...
// ── Content Flow Demo: brief → multiple outputs ───────────────────────────────
  ...
  return (
    <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'center', width: '100%' }}>
      ...
      <div style={{
        padding: '14px 28px',
        borderRadius: 12,
        border: '1.5px solid rgba(143,175,159,0.45)',
        background: 'rgba(143,175,159,0.1)',
        textAlign: 'center',
        boxShadow: '0 0 24px rgba(143,175,159,0.12)',
      }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
        ...
        <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
          <path d="M6 7L1 1M6 7l5-6" stroke="rgba(143,175,159,0.5)" strokeWidth="1.2" strokeLinecap="round" />
        </svg>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: SAGE, letterSpacing: '0.1em' }}>
          PIPELINE RUNNING<span style={{ animation: 'sp-blink 1s step-end infinite' }}>_</span>
        </div>
      </div>
        ...
        {outputs.map((out, i) => {
          ...
          return (
            <div key={out.label} style={{
              padding: '14px 16px',
              borderRadius: 10,
              border: `1px solid ${isActive ? `${out.color}30` : 'rgba(143,175,159,0.1)'}`,
              background: isActive ? `${out.color}0a` : 'rgba(28,28,26,0.4)',
              ...
              {isActive && <div style={{ marginTop: 6, width: '100%', height: 2, background: `${out.color}25`, borderRadius: 1, position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', width: '70%', background: out.color, borderRadius: 1 }} />
              </div>}
            </div>
          )
        })}
...
// ── Inbox Triage Demo: emails being sorted ────────────────────────────────────
        ...
        {emails.map((email, i) => {
          ...
          return (
            <div key={i} style={{
              padding: '11px 14px',
              borderRadius: 9,
              border: `1px solid ${isActive ? 'rgba(143,175,159,0.3)' : isProcessed ? 'rgba(143,175,159,0.1)' : 'rgba(143,175,159,0.07)'}`,
              background: isActive ? 'rgba(143,175,159,0.07)' : 'rgba(28,28,26,0.3)',
              transition: 'all 0.4s',
              opacity: !isActive && !isProcessed ? 0.3 : 1,
              display: 'flex', alignItems: 'center', gap: 10,
            }}>
              {/* Dot */}
              <div style={{
                width: 6, height: 6, borderRadius: '50%', flexShrink: 0,
                background: isProcessed ? 'transparent' : isActive ? SAGE : 'rgba(143,175,159,0.2)',
                border: isProcessed ? '1px solid rgba(143,175,159,0.2)' : 'none',
                boxShadow: isActive ? `0 0 5px ${SAGE}` : 'none',
                transition: 'all 0.4s',
              }} />
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: isActive ? SAGE : 'rgba(143,175,159,0.4)', transition: 'color 0.4s', whiteSpace: 'nowrap' }}>{email.from}</span>
                </div>
              ...
              {(isActive || isProcessed) && (
                <div style={{
                  ...
                  whiteSpace: 'nowrap',
                  flexShrink: 0,
                }}>{isProcessed ? email.action : email.tag}</div>
              )}
            </div>
          )
        })}
...
// ── Finance Flow Demo: invoice lifecycle ──────────────────────────────────────
    ...
    <div ref={containerRef} style={{ width: '100%', maxWidth: 420 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 4 }}>
        {[
          ...
          <div key={label} style={{
            ...
            borderRadius: 10,
            background: done ? `${color}0e` : active ? `${color}07` : 'rgba(28,28,26,0.03)',
            border: `1px solid ${done || active ? `${color}30` : dim}`,
      ...
      <div style={{ marginTop: 18 }}>
        <div style={{
          ...
          background: phase >= 6 ? (paid ? `${green}10` : `${red}10`) : 'rgba(28,28,26,0.04)',
          border: `1px solid ${phase >= 6 ? (paid ? `${green}30` : `${red}30`) : 'rgba(28,28,26,0.1)'}`,
          transition: 'all 0.5s',
        }}>
...
// ═════════════════════════════════════════════════════════════════════════════
// ─── SMALL CARD ANIMATIONS (reused from LocationPage pattern) ────────────────
// ═════════════════════════════════════════════════════════════════════════════
...
function IndustryCard({ industry, index }) {
  const AnimComp = CARD_ANIM_MAP[industry.animType]
  return (
...
export default function ServicePage({ service }) {
  ...
  return (
    <div style={{ backgroundColor: LIGHT, overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 3, maxWidth: 820, margin: '0 auto', width: '100%', textAlign: 'center', paddingBottom: service.heroMockup ? '56px' : 0 }}>
          ...
          <Reveal delay={0.28}>
            <a
              ...
              style={{ display: 'inline-block', fontFamily: 'DM Sans, sans-serif', fontSize: 14, fontWeight: 500, letterSpacing: '0.05em', color: '#F7F3ED', textDecoration: 'none', padding: '14px 32px', borderRadius: 9999, border: '1.5px solid rgba(143,175,159,0.75)', background: 'rgba(143,175,159,0.15)', backdropFilter: 'blur(6px)', boxShadow: '0 0 18px rgba(143,175,159,0.15), inset 0 1px 0 rgba(255,255,255,0.06)', transition: 'box-shadow 0.25s, transform 0.2s' }}
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 900, margin: '0 auto' }}>
          ...
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2rem, 4vw, 3.2rem)', fontWeight: 300, fontStyle: 'italic', color: '#F7F3ED', lineHeight: 1.1, margin: '0 0 16px', letterSpacing: '-0.02em' }}>
                {service.demoHeadline}
              </h2>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 16, fontWeight: 300, lineHeight: 1.65, color: 'rgba(232,224,208,0.5)', maxWidth: 520, margin: '0 auto' }}>
                {service.demoCopy}
              </p>
            </div>
          </Reveal>
          ...
          <Reveal delay={0.1}>
            <div style={{
              ...
              boxShadow: '0 24px 80px rgba(0,0,0,0.35), 0 0 0 1px rgba(143,175,159,0.06) inset',
      ...
      <section style={{ background: LIGHT, padding: '96px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.2rem, 4vw, 3.4rem)', fontWeight: 500, color: CHARCOAL, lineHeight: 1.1, margin: '0 auto 56px', letterSpacing: '-0.02em', maxWidth: '28ch' }}>
              {service.deliversHeadline}
            </h2>
          </Reveal>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '0 48px' }}>
            {service.whatWeDeliver.map((item, i) => (
              <Reveal key={i} delay={i * 0.05}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '18px 0', borderBottom: '1px solid rgba(212,201,176,0.4)' }}>
      ...
      <WaveDivider from={LIGHT} to={CHARCOAL} height={60} flip />
      ...
      <section style={{ background: CHARCOAL, padding: '80px 24px 96px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.2rem, 4vw, 3.4rem)', fontWeight: 300, fontStyle: 'italic', color: '#F7F3ED', lineHeight: 1.1, margin: '0 0 12px', letterSpacing: '-0.02em' }}>
              {service.industriesHeadline}
            </h2>
            <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 16, fontWeight: 300, color: 'rgba(232,224,208,0.5)', maxWidth: 540, lineHeight: 1.6, margin: '0 auto 56px' }}>
              {service.industriesCopy}
            </p>
          </Reveal>
      ...
      <section style={{ background: LIGHT, padding: '96px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.2rem, 4vw, 3.4rem)', fontWeight: 500, color: CHARCOAL, lineHeight: 1.1, margin: '0 0 12px', letterSpacing: '-0.02em' }}>
              {service.comparisonHeadline}
            </h2>
            <p style={{ fontFamily: 'DM Sans', fontSize: 16, fontWeight: 300, color: 'rgba(28,28,26,0.5)', maxWidth: 500, lineHeight: 1.6, margin: '0 auto 48px' }}>
              {service.comparisonCopy}
            </p>
          </Reveal>
          <Reveal delay={0.1}>
      ...
      <section style={{ background: PARCHMENT, padding: '96px 24px' }}>
        <div style={{ maxWidth: 780, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.2rem, 4vw, 3.4rem)', fontWeight: 500, color: CHARCOAL, lineHeight: 1.1, margin: '0 0 48px', letterSpacing: '-0.02em' }}>
              {service.faqHeadline}
            </h2>
          </Reveal>
          <div>
      ...
      <WaveDivider from={PARCHMENT} to={CHARCOAL} height={60} flip />
      ...
      <section style={{ background: CHARCOAL, padding: '96px 24px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          <Reveal>
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.4rem, 5vw, 4.2rem)', fontWeight: 300, fontStyle: 'italic', color: '#F7F3ED', lineHeight: 1.05, margin: '0 0 20px', letterSpacing: '-0.02em' }}>
              {service.ctaHeadline}
            </h2>
            <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 16.5, fontWeight: 300, lineHeight: 1.65, color: 'rgba(232,224,208,0.55)', margin: '0 0 44px', maxWidth: 500, marginLeft: 'auto', marginRight: 'auto' }}>
              {service.ctaCopy}
            </p>
            <a
              href={CTA_HREF} target="_blank" rel="noopener noreferrer"
              style={{ display: 'inline-block', fontFamily: 'DM Sans, sans-serif', fontSize: 14.5, fontWeight: 500, letterSpacing: '0.06em', color: '#1C1C1A', textDecoration: 'none', padding: '16px 40px', borderRadius: 9999, background: SAGE, boxShadow: `0 0 32px ${SAGE}30`, transition: 'transform 0.2s, box-shadow 0.2s' }}
          ...
          <div style={{ borderTop: '1px solid rgba(212,201,176,0.1)', paddingTop: 48, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
            ...
            {service.internalLinks.map(link => (
              <Link
                key={link.path}
                to={link.path}
                style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13.5, color: 'rgba(232,224,208,0.45)', textDecoration: 'none', padding: '6px 16px', borderRadius: 999, border: '1px solid rgba(212,201,176,0.14)', transition: 'all 0.2s' }}
