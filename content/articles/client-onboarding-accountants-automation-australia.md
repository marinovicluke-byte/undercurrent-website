...
- Sending and tracking engagement letters
- Collecting ATO-required forms (Tax File Number declarations, Section 396 authority forms)
- Requesting bank statements, BAS records, payroll files, superannuation summaries
- Setting up the client in Xero Practice Manager or your CRM
- Scheduling the first planning call
...
- **Tax File Number (TFN) Declaration** for new employees
- **Section 396 authority** to let you deal with the ATO on their behalf
- **Activity Statement Agent Authority** if you're lodging BAS
...
**Automated process:** Use fillable PDF forms with pre-filled fields (business name, ABN, your agent number) sent via DocuSign or an e-signature tool. They fill in the blanks digitally. The form auto-saves to your practice management system. Your [workflow automation](/services) checks that required fields are complete before marking the client as "Ready to Lodge".
...
- The form collects business name, ABN, contact name, email, phone, and service type (tax return, bookkeeping, BAS lodgement).

**Step 2: System sends engagement letter via DocuSign.**
- The engagement letter is pre-filled with their business name and service details.
- DocuSign sends them a signing link by email and SMS.

**Step 3: Client signs the engagement letter.**
- DocuSign triggers an n8n workflow.
- n8n creates a client record in Xero Practice Manager with all details from the form.
- n8n sends the client a welcome email with a secure upload link.

**Step 4: Client uploads required documents.**
- The upload portal has a checklist: BAS statements, bank statements, payroll summaries, super statements.
- Files are automatically saved to Google Drive in a folder named `[Client Name] - Onboarding`.
- File names are standardised (e.g., `ClientName_BAS_Q1_2026.pdf`).

**Step 5: System notifies the accountant.**
- When all documents are uploaded, n8n sends a Slack message: "New client onboarded: [Client Name]. Documents ready for review."
- Xero Practice Manager updates the client status to "Ready for Review".

**Step 6: Accountant approves and schedules first call.**
- You log into Xero, review the documents, and click "Approve".
- Your system sends the client a calendar invite for the planning call and a confirmation email.
...
| Tool Type | Example | Cost (AUD/month) | What It Does |
|-----------|---------|------------------|--------------|
| E-signature | DocuSign, PandaDoc | $15-$50 | Engagement letters, ATO forms |
| Document portal | Ignition, Karbon, custom form | $50-$150 | Secure file uploads with checklists |
| Workflow automation | n8n, Zapier, Make | $0-$50 | Connects your tools, no-code workflows |
| Practice management | Xero Practice Manager | $100-$150 | Client records, job tracking, compliance |
...
**Option 1: DIY with no-code tools (2-4 weeks, $0-$500 setup cost).**
If you're comfortable with tools like Zapier or Make, you can connect DocuSign, Google Drive, and Xero Practice Manager yourself. It'll take some trial and error, but it's doable. The risk: you'll spend 2-4 weeks learning, testing, and fixing broken workflows when a field name changes. That's 2-4 weeks you're not billing clients.

**Option 2: Use an all-in-one platform (1 day setup, $150-$300/month ongoing).**
Tools like Ignition or Karbon are built specifically for accounting firms. They have client onboarding workflows out of the box. Less flexibility than a custom build, but faster to launch. Good choice if you want something working by next week.

**Option 3: Work with an automation specialist (1-2 weeks, $2,000-$8,000 one-time build cost).**
You describe your current onboarding process. An automation specialist (like [UnderCurrent](/about)) maps it out, builds the workflows, connects your tools, and tests everything with real client data. You get a system tailored to how your firm actually works,not how a generic SaaS platform thinks you should work. The system lives inside your tools, not bolted on top. If you change tools later, we migrate the workflows. It's a [partnership, not a transaction](/process).
...
- **Pre-filled fields:** Your system fills in your agent number, the client's ABN, and the date automatically. They only fill in the blanks you can't pre-fill (like their TFN).
- **Required field validation:** The form won't submit unless all required fields are complete. No more "oops, they forgot to sign" moments.
- **Version control:** You're always using the current ATO form template. No risk of sending a client an outdated form you saved to your desktop in 2019.
...
If compliance forms are a blocker for you, that's exactly where [custom workflow automation](/services) pays off. We build it once, test it with your client data, and you never think about it again.
...
Want to see what a system like this could save your practice? Use our [free business audit](/audit) to map your current onboarding workflow and spot the biggest time leaks.
...
Write down every step from "new client says yes" to "we're ready to start billable work". List every email, every follow-up, every file you touch.

**Step 2: Pick the highest-impact task.**
Which step wastes the most time or causes the most frustration for you or the client? Start there.

**Step 3: Automate it.**
Use a tool (DocuSign, Ignition, n8n) or [work with us](/contact) to build the workflow. Test it with one client. Refine. Then roll it out to all new clients.
...
**Ready to stop chasing signed PDFs and start onboarding clients in days, not weeks?** [Book a free business audit](/audit) and we'll map your current onboarding process, spot the biggest time leaks, and show you exactly which workflows to automate first. Our one goal is for you to be our best case study.
